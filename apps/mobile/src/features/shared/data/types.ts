...
/**
 * The roles a user may hold (migration 0040). A user holds a SET of these, not
 * one of them — `technician` and `customer` together is the normal case, not an
 * edge case (§5.1.1).
 *
 * This type describes what the server said, never what the client decided. The
 * app uses it to choose what to render; what a user may *do* is decided by RLS
 * (§5.1.3).
 */
export type UserRole = 'customer' | 'technician' | 'workshop_admin' | 'ops' | 'super_admin';

/** `none` = never applied. Anything else mirrors providers.verification_status. */
export type ProviderApplicationStatus =
  'none' | 'pending' | 'in_review' | 'approved' | 'rejected' | 'suspended';
